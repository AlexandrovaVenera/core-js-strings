...
function countVowels(str) {
  ...
  return str
    .split('')
    .filter((current) => vowels.includes(current.toLocaleLowerCase())).length;
}
...
function isPalindrome(str) {
  return (
    str.replaceAll(/\W/g, '').toLowerCase().split('').reverse().join('') ===
    str.replaceAll(/\W/g, '').toLowerCase().split('').join('')
  );
}
...
function findLongestWord(sentence) {
  return sentence
    ...
    .find(
      (word) =>
        word.length === Math.max(...sentence.split(' ').map((el) => el.length))
    );
}
